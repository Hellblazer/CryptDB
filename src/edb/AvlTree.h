...
class AvlTree
{
 public:
    ...
    // These methods find the element immediately to the left (sequentially
    // previous)
    // or right (sequentially next) of a given element.
    //
    const T & rootElement() const
    throw (NotFound);
    const T & prevElement( const T & x) const
    throw (NotFound);
    const T & nextElement( const T & x) const
    throw (NotFound);
    ...
    //
    // Otherwise, only the array must be deallocated and not the pointers
    // returned
    // inside it
    //
    void getAllElements(vector<T*>& elements, bool copy = true) const;

    // Returns all ancestors of a given element, terminated with a NULL
    // element.
    // The caller is responsible for deallocation of the array and all
    // containing elements.
    //
    void getAllAncestors(vector<T*>& ancestors, const T& x, bool copy =
                             true) const;

    // This function signature is used for callbacks when performing range
    // queries
    //
    ...
    // and max inclusive
    //
    void rangeQuery(vector<T*>& range, const T& min, const T& max,
                    bool copy = true) const;

    // A range query that calls a callback method on each match
    //
    void rangeQuery(RangeQueryCallback cb, const T& min, const T& max,
                    void* arg) const;

    // Range query that returns another tree
    //
    //void rangeQuery(AvlTree<T,Compare,Distance>& range, const T& min, const
    // T& max) const;
 ...
 private:
    ...
    const T & elementAt( const AvlNode<T,Compare,Distance> *t ) const
    throw (NotFound);
    const T * const elementPtrAt( const AvlNode<T,Compare,
                                                Distance> *t ) const;
    ...
    void printTree( ostream& out,
                    AvlNode<T,Compare,Distance> *t, int numTabs = 0,
                    char lr = '_' ) const;

    // Merging two branches
    //
    bool merge( AvlNode<T,Compare,Distance>* b, AvlNode<T,Compare,
                                                        Distance>* &t);
    ...
    AvlNode<T,Compare,Distance> * find( const T & x, AvlNode<T,Compare,
                                                             Distance> *t )
    const;
    AvlNode<T,Compare,Distance> * findClosest( const T & x, AvlNode<T,Compare,
                                                                    Distance>
                                               *t ) const;
    ...
    //
    void replaceNode(AvlNode<T,Compare,Distance>* &node1, AvlNode<T,Compare,
                                                                  Distance>*
                     &node2);
    ...
    //
    void getAllElements(vector<T*>& elements,
                        AvlNode<T,Compare,Distance>* root,
                        bool copy) const;

    // Returns all ancestors of a given element
    //
    void getAllAncestors(vector<T*>& ancestors,
                         AvlNode<T,Compare,Distance>* node,
                         bool copy) const;

    // Returns a range query for all of the elements that lie within the min
    // and max inclusive
    //
    void rangeQuery(vector<T*>& range,
                    AvlNode<T,Compare,
                            Distance>* root, const T& min, const T& max,
                    bool copy) const;

    // Range query that calls a callback method on each match
    //
    void rangeQuery(RangeQueryCallback,
                    AvlNode<T,Compare,
                            Distance>* root, const T &min, const T &max,
                    void* arg) const;
    ...
    //
    //void rangeQuery(AvlTree<T,Compare,Distance>& range,
    // AvlNode<T,Compare,Distance>* root, const T& min, const T& max) const;

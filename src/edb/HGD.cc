...
/**
* THIS IS A .C IMPLEMENTATION OF A FORTRAN IMPLEMENTATION WITH THE SPECIFICATION GIVEN IN THE COMMENT BELOW.
* I replaced some helper functions with more appropriate ones and adapted the code to work with very large numbers 
...
C        RAND(): A UNIFORM (0,1) RANDOM NUMBER GENERATOR. -- since it is only once invoked for seed, I just return seed
C
C     ARGUMENTS
...
*/
      double AL[8] = {0.0, 0.0, 0.6931471806, 1.791759469, 3.178053830, 4.787491743, 6.579251212, 8.525161361};
...
RR randomValue(ZZ & seed, unsigned int seedLen) {
...
ZZ HGD(ZZ KK, ZZ  NN1, ZZ  NN2, ZZ  SEED, unsigned int seedLen, unsigned int RRPrecision)  {
...
*/
      ...
      if (  (NN1 <  0) ||
           ...
           (KK < 0)  ||
           (KK > NN1 + NN2 )  ) {
	  cerr << "invalid parameters NN1 " << NN1 << " NN2 " <<  NN2 << " KK " << KK << "\n";
...
*/
	 ...
	 return to_ZZ(MAXJX);
      ...
      } else if (M-MINJX < 10) {//won't really happen in OPE cause M will be on the order of N1
/*
...
/* 10 */
		...
		if (DEBUG) {
		    cerr << "passed through label ten " << countFlagTen << " times \n";
		}
	    }
	    flagTen = false;
	    IX = MINJX;
    ...
    /* 20 */   
		...
		if (countFlagTwenty > 1000) {
		    myassert(false, "passed through label twenty more than 1000 times...\n");
		}
		flagTwenty = false;
		if (U > P)  {
		    U  = U - P;
...
*/
            ...
            RR XK  = K - M + 0.5;
            NM  = N2 - K + XM;
            RR UB  = Y * GU - M * GL + DELTAU  + XM * R * (1.+R*(-.5+R/3.))  + XN * S * (1.+S*(-0.5+S/3))  + XK * T * (1.+T*(-.5+T/3.))   + NM * E * (1.+E*(-.5+E/3.));
/*

...
#define DECRYPT_int_sem "decrypt_int_sem( bigint, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, bigint)"
#define DECRYPT_int_det "decrypt_int_det( bigint, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer)"
#define DECRYPT_text_sem "decrypt_text_sem( bytea, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, bigint)"
...
void dropAll(Connect * conn) {

cerr << "trying to drop\n";
	myassert(conn->execute( "DROP FUNCTION IF EXISTS " DECRYPT_int_sem "; "), "cannot drop " DECRYPT_int_sem);
	cerr << "dropped decrypt int sem \n";
	myassert(conn->execute( "DROP FUNCTION IF EXISTS " DECRYPT_int_det "; "), "cannot drop " DECRYPT_int_det);
	myassert(conn->execute( "DROP FUNCTION IF EXISTS " ENCRYPT_int_det "; "), "cannot drop " ENCRYPT_int_det);
	myassert(conn->execute( "DROP FUNCTION IF EXISTS " DECRYPT_text_sem "; "), "cannot drop " DECRYPT_text_sem);
	myassert(conn->execute( "DROP FUNCTION IF EXISTS " SEARCH "; "), "cannot drop " SEARCH);
	myassert(conn->execute( "DROP FUNCTION IF EXISTS " FUNC_ADD_FINAL "; "), "cannot drop " FUNC_ADD_FINAL);
	myassert(conn->execute( "DROP FUNCTION IF EXISTS " FUNC_ADD_SET "; "), "cannot drop " FUNC_ADD_SET);


#if MYSQL_S
#else
	myassert(conn->execute( "DROP FUNCTION IF EXISTS " FUNC_ADD "; "), "cannot drop "FUNC_ADD);
    myassert(conn->execute("DROP AGGREGATE IF EXISTS agg_sum(bytea) ; "), "cannot drop agg_sum");
#endif
...
void createAll(Connect * conn, CryptoManager * cm) {
	...
	myassert(conn->execute("CREATE FUNCTION decrypt_text_sem RETURNS STRING SONAME 'edb.so'; "),"failed to create udf decrypt_text_sem");
	myassert(conn->execute("CREATE FUNCTION search RETURNS INTEGER SONAME 'edb.so'; "),"failed to create udf search");
...
const char * createInfoTable (CryptoManager * cm) {
	string res = "CREATE TABLE PKINFO (n2 bytea); INSERT INTO PKINFO VALUES (";
...
void EDBClient::createEncryptionTables(int noOPE, int noHOM) {

	list<string> fieldsWithOPE = list<string>();

	for (map<string, TableMetadata *>::iterator tit = tableMetaMap.begin(); tit != tableMetaMap.end(); tit++) {
		TableMetadata * tm = tit->second;
		for (map<string, FieldMetadata *>::iterator fit = tm->fieldMetaMap.begin(); fit != tm->fieldMetaMap.end(); fit++) {
			FieldMetadata * fm = fit->second;
			if (fm->exists(fm->anonFieldNameOPE)) {
				fieldsWithOPE.push_back(fullName(fm->anonFieldNameOPE, tm->anonTableName));
			}
		}
	}
...
void EDBClient::replenishEncryptionTables() {
	cm->replenishEncryptionTables();
}


fieldType getType(list<string>::iterator & it, list<string> & words) {
...
list<const char *> EDBClient::processIndex(list<string> & words, list<string>::iterator & wordsIt) throw (SyntaxError) {
	...
	while (wordsIt->compare(")") != 0) {
		...
		string anonFieldName = getOnionName(tableMetaMap[table]->fieldMetaMap[fieldName], oDET);
...
int getExprFromWhere(list<string>::iterator & it, list<string>::iterator end, QueryMeta & qm, string & a, string & op, string & b, map<string, TableMetadata *> & tableMetaMap) {
...
string getNameForFilter(FieldMetadata * fm, onion o) {
	if (!fm->isEncrypted) {
		return fm->fieldName;
	}
	if (o == oDET) {
		//for equality type operations use OPE if you can
		if (fm->exists(fm->anonFieldNameOPE) && (fm->secLevelOPE != SEMANTIC_OPE)) {
...
void processAnnotation(MultiPrinc * mp, list<string>::iterator & wordsIt, list<string> & words, string tableName, string fieldName, bool & encryptfield, map<string, TableMetadata *> & tm) {
	if (DECRYPTFIRST) {
			encryptfield = true;
			tm[tableName]->fieldMetaMap[fieldName]->secLevelDET = DETJOIN;
			//make ope onion uncovered to avoid adjustment --- though this onion does not exist in this mode
	...
	mp->processAnnotation(wordsIt, words, tableName, fieldName, encryptfield, tm);
	cerr << "done with multi princ \n";
}


list<const char*> EDBClient::rewriteEncryptCreate(const char* query) throw (SyntaxError){
	...
	if (toLowerCase(*wordsIt).compare("table") != 0) {
		...
		//index
		assert_s(equalsIgnoreCase(*wordsIt,"index") || equalsIgnoreCase(*wordsIt,"unique"), "expected index/unique ");
	...
	assert_s(tableMetaMap.find(tableName) == tableMetaMap.end(),"table already exists: " + tableName );
	...
	while (wordsIt != words.end()) {
	    ...
	    assert_s(tm->fieldMetaMap.find(fieldName) == tm->fieldMetaMap.end(), "field %s already exists in table " +  fieldName + " " + tableName);
		...
		processAnnotation(mp, wordsIt, words, tableName, fieldName, encryptfield, tableMetaMap);
		...
		if (!encryptfield) {
			...
			fieldSeq +=  fieldName + " " + mirrorUntilTerm(wordsIt, words, terms, 1, 1, 1);
			continue;
		}

		fm->type = getType(wordsIt, words);

		fieldSeq += processCreate(fm->type, fieldName, i, encryptfield, tm, fm) + " ";
...
ResType * EDBClient::rewriteDecryptCreate(const char * query, ResType * dbAnswer) {
	if (VERBOSE) {cout << "Raw server response: \nCREATE TABLE; \n\n\n";}
	return new ResType(); //no decryption performed, no need to waste time
}

ResType * EDBClient::rewriteDecryptDelete(const char * query,  ResType * dbAnswer) {
	if (VERBOSE) {cout << "DELETED\n";}
	return new ResType();
}

//TODO: MULTIPRINC does not have update fully implemented
list<const char*> EDBClient::rewriteEncryptUpdate(const char * query) throw (SyntaxError){

	//	UPDATE folders SET mitgeec = 'DOC', mitgeecs_app_term = '9/2007' WHERE id = '99e89298fa'
	...
	//table
	assert_s(tableMetaMap.find(*wordsIt) != tableMetaMap.end(), " table to update does not exist" );
	...
	while ((wordsIt != words.end()) && (!equalsIgnoreCase(*wordsIt,"where"))) {
		string table, field;
		...
		if (!tableMetaMap[table]->fieldMetaMap[field]->isEncrypted) {
			resultQuery = resultQuery + " " + field +  mirrorUntilTerm(wordsIt, words, term, noTerms, 0, 1);
		...
		if (isField(*wordsIt)) { //this is an increment
			...
			tableMetaMap[table]->fieldMetaMap[field]->INCREMENT_HAPPENED = true;
			string anonFieldName = getOnionName(tableMetaMap[table]->fieldMetaMap[field], oAGG);
			...
			string anonName2 = fieldNameForQuery(tableMetaMap[table2]->anonTableName, table2,
					getOnionName(fm, oAGG),fm->type, qm);

			wordsIt++;
			bool isEncrypted1 =  tableMetaMap[table]->fieldMetaMap[field]->isEncrypted;
			assert_s( isEncrypted1 == tableMetaMap[table2]->fieldMetaMap[field2]->isEncrypted, "must both be encrypted or not\n");
			assert_s((!isEncrypted1) || wordsIt->compare("+") == 0, "can only update with plus");
			string op = *wordsIt;
			...
			if (isEncrypted1) {
				...
				if (DECRYPTFIRST) {
					resultQuery += field + " =  encrypt_int_det( "+ fieldNameForQuery(tableMetaMap[table]->anonTableName, table, field2, TYPE_INTEGER, qm) +  " + " + *wordsIt + ", "+ CryptoManager::marshallKey(dec_first_key) + ") ";
				} else {
		...
		if (ft == TYPE_INTEGER) {
			...
			SECLEVEL sldet = tableMetaMap[table]->fieldMetaMap[field]->secLevelDET;
			string anonfieldName = getOnionName(tableMetaMap[table]->fieldMetaMap[field], oDET);

			resultQuery = resultQuery + anonfieldName  + " = " ;

			assert_s(tableMetaMap[table]->fieldMetaMap[field]->type == TYPE_INTEGER, "update not yet supported for text - integer only \n");
			...
			if (DECRYPTFIRST) {
				...
				if (sldet == SEMANTIC_DET) {
					...
					resultQuery += crypt(val, TYPE_INTEGER, fullName(field, table), fullName(anonfieldName, anonTableName), PLAIN_DET, DET, 0, tmkm);

				} else {
					resultQuery = resultQuery +
							crypt(val, TYPE_INTEGER, fullName(field, table), fullName(anonfieldName, anonTableName), PLAIN_DET, sldet, 0, tmkm);
				}

				if (FieldMetadata::exists(tableMetaMap[table]->fieldMetaMap[field]->anonFieldNameOPE)) {

					SECLEVEL slope = tableMetaMap[table]->fieldMetaMap[field]->secLevelOPE;
					anonfieldName = tableMetaMap[table]->fieldMetaMap[field]->anonFieldNameOPE;
					resultQuery = resultQuery  + ", " + anonfieldName + " = " ;
					if (slope == SEMANTIC_OPE) {
						addIfNotContained(fullName(field, table), fieldsDec.OPEFields);
					}
					resultQuery = resultQuery +
							crypt(val, TYPE_INTEGER, fullName(field, table), fullName(anonfieldName, anonTableName), PLAIN_OPE, OPESELF, 0, tmkm);

				}
				if (FieldMetadata::exists(tableMetaMap[table]->fieldMetaMap[field]->anonFieldNameAGG)) {
					anonfieldName = tableMetaMap[table]->fieldMetaMap[field]->anonFieldNameAGG;
					resultQuery = resultQuery  + ", " + anonfieldName + " = " +
							crypt(val, TYPE_INTEGER, fullName(field, table), fullName(anonfieldName, anonTableName), PLAIN_AGG, SEMANTIC_AGG, 0, tmkm);
				}
			}
		}
		if (ft == TYPE_TEXT) {

			if (tableMetaMap[table]->fieldMetaMap[field]->secLevelDET == SEMANTIC_DET) {
				addIfNotContained(fullName(field, table), fieldsDec.DETFields);
			}


			assert_s(hasApostrophe(*wordsIt), "missing apostrophe for type text");

			if (DECRYPTFIRST) {
				resultQuery = resultQuery + " "  + field + " = " +
						processValsToInsert(field, table, 0, *wordsIt, tmkm);
			} else {
				//encrypt the value
				string anonfieldname = getOnionName(tableMetaMap[table]->fieldMetaMap[field], oDET);
				...
				if (tableMetaMap[table]->fieldMetaMap[field]->isEncrypted) {
					resultQuery = resultQuery + " "  + anonfieldname + " = " +
							crypt(*wordsIt, TYPE_TEXT, fullName(field, table),  fullName(anonfieldname, anonTableName), PLAIN_DET, DET, 0, tmkm);
				} else {
					resultQuery = resultQuery + " " + anonfieldname + " = " + *wordsIt;
				}
	...
	list<const char *>  res =processFilters(wordsIt, words, qm,  resultQuery, fieldsDec, tmkm);
...
ResType * EDBClient::rewriteDecryptUpdate(const char * query, ResType * dbAnswer)  {
	return new ResType; //no decryption performed, no need to waste time
}
...
bool isOp(string token, string operators) {
...
list<const char*>  EDBClient::processFilters(list<string>::iterator &  wordsIt, list<string> & words, QueryMeta & qm,  string resultQuery, FieldsToDecrypt fieldsDec, TMKM & tmkm, list<const char *> subqueries) throw (SyntaxError) {
	...
	while (equalsIgnoreCase(*wordsIt,"left")) {
		...
		wordsIt++;
		assert_s(equalsIgnoreCase(*wordsIt,"join"), "expected left join, got left only");
		wordsIt++;
		//this goes up to on
		resultQuery += mirrorUntilTerm(wordsIt, words, parserMeta.querySeparators_p, 1, 1);
		//goes up to next separator
		resultQuery += mirrorUntilTerm(wordsIt, words, parserMeta.querySeparators_p, 0, 1);
		if (wordsIt == words.end()) {
			resultQuery += ";";
			goto decryptions;
		}
	...
	while ((wordsIt != words.end()) && (!isQuerySeparator(*wordsIt))) {
		...
		processDecryptionsForOp(op, operand1, operand2, fieldsDec, qm, tableMetaMap);
		string subquery = "*";
		...
		resultQuery = resultQuery + processOperation(op, operand1, operand2,  qm, subquery, tmkm);
		wordsIt++;
	}

	if (VERBOSE) { 	if (wordsIt != words.end()) {cerr << "next is " << *wordsIt << " \n";}}
	groupings:
	...
	while ((wordsIt != words.end()) && (equalsIgnoreCase(*wordsIt,"order") || equalsIgnoreCase(*wordsIt,"group"))) {
			string comm = *wordsIt;
			resultQuery += " " + *wordsIt; wordsIt++;
			assert_s(equalsIgnoreCase(*wordsIt, "by"), "BY should come after " + comm);
			resultQuery += " " + *wordsIt; wordsIt++;
			...
			while ((wordsIt != words.end()) && isField(*wordsIt)) {
				...
				resultQuery = resultQuery + " " + fieldNameForQuery(tableMetaMap[table]->anonTableName, table, anonField, fm->type, qm);
				...
				if ((wordsIt != words.end()) &&
						(equalsIgnoreCase(*wordsIt,"asc") || equalsIgnoreCase(*wordsIt,"desc"))) {
	...
	assert_s(wordsIt == words.end(), "expected end of query at token, but query is not done ");
...
list<const char*>  EDBClient::processDecryptions(FieldsToDecrypt fieldsDec, TMKM & tmkm) throw (SyntaxError) {
	...
	for (list<string>::iterator it = fieldsDec.DETFields.begin(); it != fieldsDec.DETFields.end(); it++) {
		string table, field;
		getTableField(*it, table, field, qm, tableMetaMap);
		...
		if (MULTIPRINC) {
			assert_s(false, "there should be no adjustment in multi-key mode\n");
		}
		...
		/*if (MULTIPRINC) {
			assert_s((mkm.encForMap.find(fname) != mkm.encForMap.end()) && (tmkm.encForVal.find(fname) != tmkm.encForVal.end()), "internal: process decryptions missing data for encrypted field");
			whereClause = " WHERE " + getField(mkm.encForMap[fname]) + " = " + tmkm.encForVal[mkm.encForMap[fname]] + whereClause;
		}*/
		...
		switch (ft) {
		case TYPE_INTEGER: {
			decryptS = "decrypt_int_sem(" + anonfieldName + "," + cm->marshallKey(cm->getKey(fullName(anonfieldName, anonTableName), SEMANTIC_DET)) + ", " + "salt)" + whereClause ;
			//cout << "KEY USED TO DECRYPT field from SEM " << anonfieldName << " " << cm->marshallKey(cm->getKey(anonTableName +"."+anonfieldName, SEMANTIC)) << "\n"; fflush(stdout);
			break;
		}
		case TYPE_TEXT: {
			decryptS = "decrypt_text_sem(" + anonfieldName + "," + cm->marshallKey(cm->getKey(fullName(anonfieldName, anonTableName), SEMANTIC_DET)) + ", " + "salt)" + whereClause;
			break;
		}
		...
		string resultQ = string("UPDATE ") + tableMetaMap[table]->anonTableName +
	...
	for (list<string>::iterator it = fieldsDec.OPEFields.begin(); it != fieldsDec.OPEFields.end(); it++) {
		string table, field;
		getTableField(*it, table, field, qm, tableMetaMap);
		...
		string decryptS = "decrypt_int_sem("  + anonfieldName + "," +
				cm->marshallKey(cm->getKey(fullName(anonfieldName, anonTableName), SEMANTIC_OPE)) +
				", " +  "salt)" + whereClause;
	...
	for (list<string>::iterator it = fieldsDec.DETJoinFields.begin(); it != fieldsDec.DETJoinFields.end(); it++) {
		...
		case TYPE_INTEGER: {
			...
			//cout << "KEY USED TO DECRYPT field from DET " << anonfieldName << " " << cm->marshallKey(cm->getKey(anonTableName +"."+anonfieldName, DET)) << "\n"; fflush(stdout);
			decryptS = "decrypt_int_det(" + anonfieldName + "," + cm->marshallKey(cm->getKey(fullName(anonfieldName, anonTableName), DET)) + ");"+'\0';
			break;
		}
		case TYPE_TEXT: {
	...
	for (list<string>::iterator it = fieldsDec.OPEJoinFields.begin(); it != fieldsDec.OPEJoinFields.end(); it++) {
		string table, field;
		getTableField(*it, table, field, qm, tableMetaMap);

		//for now, do nothing
		//result.push_back(getCStr(string("UPDATE ") + tableMetaMap[table]->anonTableName +
		...
		if (MULTIPRINC) {
			assert_s(false, " opejoin adjustment not supported with multi-user\n");
		}
	}
...
list<list<string> > getSimpleQ(const char * query) {
...
list<string> getAnonNames(FieldMetadata * fm) {
...
list<const char*> EDBClient::rewriteEncryptSelect(const char * query) throw (SyntaxError){
	...
	for (list<list<string> >::iterator qit = simpleQ.begin(); qit != simpleQ.end(); qit++) {
	...
	for (list<list<string> >::iterator qit = simpleQ.begin(); qit != simpleQ.end(); qit++) {
		...
		//cerr << "helper result "; myPrint(auxqueries); cerr << "\n";
		assert_s(auxqueries.size() == 1, " aux queries should be one in size after first pass ");
	...
	list<const char *> results = rewriteSelectHelper(simpleQ.back(), false, encqueries);
	//cerr << "helper result "; myPrint(results); cerr << "\n";
	assert_s(results.size() == 1, "there should not be any new decryption queries\n");
...
void expandWildCard(list<string> & words, QueryMeta & qm, map<string, TableMetadata *> & tableMetaMap) {
	...
	while (!equalsIgnoreCase(*wordsIt, "from")) {
		...
		if (isAgg(*wordsIt)) {
			...
			onion ox;
			processAgg(wordsIt, words, fieldx, tablex, ox, qm, tableMetaMap, 0);
			continue;
		}
		//case 1: *
		if (wordsIt->compare("*") == 0) {
			int noinserts = 0;
			for (list<string>::iterator tit = qm.tables.begin(); tit != qm.tables.end() ; tit++) {
				//add comma first if some field names have been inserted already
				if (noinserts > 0) {
				...
				int index = 0;
				for (list<string>::iterator fieldsIt = fnames.begin() ; fieldsIt != fnames.end() ; fieldsIt++ ) {
					FieldMetadata * fm = tm->fieldMetaMap[*fieldsIt];
					words.insert(wordsIt, fieldNameForQuery(*tit, *tit, *fieldsIt, fm->type, qm, 1));

					index++;
					if (index < count) { //add comma only if there are more fields to come
						words.insert(wordsIt, ",");
					}
					noinserts++;
		...
		if (hasWildcard(*wordsIt)) {
			...
			for (list<string>::iterator fieldsIt = fnames.begin() ; fieldsIt != fnames.end() ; fieldsIt++ ) {
				...
				FieldMetadata * fm = tm->fieldMetaMap[*fieldsIt];
				words.insert(wordsIt,  fieldNameForQuery(table, table, *fieldsIt, fm->type, qm, 1));
				index++;
				if (index < count) { //add comma only if there are more fields to come
					words.insert(wordsIt, ",");
				}
...
list<const char*> EDBClient::rewriteSelectHelper(list<string> words, bool isSubquery, list<const char *> subqueries) throw (SyntaxError){
...
//	gettimeofday(&endtime, NULL);
//		cout << "MULTIPRINC prepare select" << timeInMSec(starttime, endtime) << "\n";
	...
	while (!equalsIgnoreCase(*wordsIt, "from")) {
		...
		string table, field;
     	...
     	if (toLowerCase(*wordsIt).compare("sum") == 0) {
			wordsIt++;
			assert_s(wordsIt->compare("(") == 0, "sum should be followed by parenthesis");
			wordsIt++;
			...
			string res = mirrorUntilTerm(wordsIt, words, termin, noTermin, 0, 0);

			assert_s(wordsIt->compare(")") == 0, ") needed to close sum expression\n  ");
			wordsIt++;
			...
			if (fm->isEncrypted) {
				...
				if (DECRYPTFIRST) {
					resultQuery += " sum(" + fieldNameForQuery(tm->anonTableName, table, field, fm->type, qm) +") ";
				} else {
					funcname = SUM_AGG;
				...
				if (MULTIPRINC) {
					resultQuery += mp->selectEncFor(table, field, qm, tmkm, tm, fm);
				}

			} else {

				funcname = "sum";
				resultQuery = resultQuery + " " + funcname + "( "  + field + res + " ) ";
			}
     	...
     	if (equalsIgnoreCase(*wordsIt,"count")){
     		...
     		resultQuery += *wordsIt;
     		wordsIt++;
     		assert_s(wordsIt->compare("(") == 0, "missing ( in count expression \n");
     		resultQuery += *wordsIt;
     		wordsIt++;
     		...
     		if (toLowerCase(*wordsIt).compare("distinct") == 0) {
     			...
     			FieldMetadata * fmd = tableMetaMap[tableD]->fieldMetaMap[fieldD];
     			if (fmd->secLevelDET == SEMANTIC_DET) {
     				addIfNotContained(fullName(fieldD, tableD), fieldsDec.DETFields);
     			}
     			if (DECRYPTFIRST) {
     				resultQuery  += " " + fieldNameForQuery(tableMetaMap[tableD]->anonTableName, tableD, fieldD, fmd->type, qm);

     			} else {
     				resultQuery  += " " + fieldNameForQuery(tableMetaMap[tableD]->anonTableName, tableD, getNameForFilter(fmd, oDET), fmd->type, qm);
     			}
     			wordsIt++;
     			resultQuery += *wordsIt;
     			myassert(wordsIt->compare(")") ==0, "expected ) after field for DISTINCT \n");
     			wordsIt++;
     			if (wordsIt->compare(")") == 0) {
     				resultQuery += *wordsIt;
     				wordsIt++;
     			...
     			if (wordsIt->compare("*") !=0 ) {
     				...
     				FieldMetadata * fmd = tableMetaMap[tableD]->fieldMetaMap[fieldD];
     				resultQuery +=
     						fieldNameForQuery(tableMetaMap[tableD]->anonTableName, tableD, getNameForFilter(fmd,oDET), fmd->type, qm) ;
     				wordsIt++;
     				resultQuery += *wordsIt;
     				myassert(wordsIt->compare(")") ==0, "expected ) after field for COUNT \n");
		...
		if (equalsIgnoreCase(*wordsIt, "max") || equalsIgnoreCase(*wordsIt, "min")) {

			resultQuery = resultQuery + " " + *wordsIt +"( ";
			wordsIt++;
			assert_s(wordsIt->compare("(") == 0, "missing ( in max/min expression \n");
			wordsIt++;
			...
			string anonName = getOnionName(tableMetaMap[table]->fieldMetaMap[field],oOPE);
			...
			if (fm->isEncrypted) {
				if (DECRYPTFIRST) {
					resultQuery = resultQuery + fieldNameForQuery(tm->anonTableName, table, field, fm->type, qm);
				} else {
					resultQuery = resultQuery + fieldNameForQuery(tm->anonTableName, table, anonName, fm->type, qm);
				}
				...
				if (tableMetaMap[table]->fieldMetaMap[field]->secLevelOPE == SEMANTIC_OPE) {
					addIfNotContained(fullName(realname, table), fieldsDec.OPEFields);
				}
			} else {
				resultQuery += *wordsIt;
			}
			...
			wordsIt++;
			assert_s(wordsIt->compare(")") == 0, " missing ) in max expression \n");
			wordsIt++;
			resultQuery = resultQuery + processAlias(wordsIt, words);
			continue;
		}
		...
		if (!DECRYPTFIRST) {
			...
			if (table.compare(oldTable)) {
				...
				if (tm->hasEncrypted && (!isSubquery)) {
					resultQuery = resultQuery + " " + fieldNameForQuery(tm->anonTableName, table, "salt", fm->type, qm) + " ,";
				}
			}
		}

		if (!fm->isEncrypted) {
			resultQuery = resultQuery + " " + fieldNameForQuery(tm->anonTableName, table, field,fm->type,  qm) + processAlias(++wordsIt, words);
			continue;
		}
		...
		if (isSubquery) {
			resultQuery = resultQuery + " " +
					fieldNameForQuery(tm->anonTableName, table, getOnionName(tm->fieldMetaMap[field], oDET), fm->type, qm) ;
			...
			if (tm->fieldMetaMap[field]->secLevelDET == SEMANTIC_DET) {
				addIfNotContained(fullName(field, table), fieldsDec.DETFields);
				addIfNotContained(fullName(field, table), fieldsDec.DETJoinFields);
			}
			...
			if (DECRYPTFIRST) {
				resultQuery = resultQuery + " " +
					fieldNameForQuery(tm->anonTableName, table, field, fm->type, qm);
			} else {
				resultQuery = resultQuery + " " +
									fieldNameForQuery(tm->anonTableName, table, anonFieldNameForDecrypt(fm), fm->type, qm);
			}
			if (MULTIPRINC) {
				resultQuery += mp->selectEncFor(table, field, qm, tmkm, tm, fm);
			}
	...
	while ((wordsIt != words.end()) && (!isQuerySeparator(*wordsIt))) {
		...
		assert_s(tableMetaMap.find(*wordsIt) != tableMetaMap.end(), "table " + *wordsIt + "does not exist");
		resultQuery = resultQuery + " " + tableMetaMap[*wordsIt]->anonTableName;
		wordsIt++;
		resultQuery = resultQuery + processAlias(wordsIt, words);
	}

	list<const char*> res = processFilters(wordsIt, words, qm, resultQuery, fieldsDec, tmkm, subqueries);
...
ResMeta getResMeta(list<string> words, vector<vector<string> > & vals, QueryMeta & qm, map<string, TableMetadata * > & tm, MultiPrinc * mp, TMKM & tmkm) {
	...
	for (unsigned int i = 0; i < nFields; i++) {
		...
		if (isAgg(*wordsIt)) {
			...
			rm.namesForRes[i] = processAgg(wordsIt, words, rm.field[i], rm.table[i], rm.o[i], qm, tm,0);
			if (MULTIPRINC) {
				mp->processReturnedField(i, fullName(rm.field[i], rm.table[i]), rm.o[i], tmkm, ignore);
			}

//			cerr << "field, " << rm.field[i] << " table  " << rm.table[i] << " onion " << rm.o[i] << " nameForRes " << rm.namesForRes[i] << "\n";
			continue;
		}
...
void printRes(vector<vector<string> > & vals) {
...
ResType * EDBClient::rewriteDecryptSelect(const char * query, ResType * dbAnswer) {
...
//cerr << "going in a tuple! \n";
		...
		for (unsigned int j = 0; j < nFields; j++) {
			...
			string fullAnonName = fullName(getOnionName(fm, rm.o[j]), tableMetaMap[table]->anonTableName);
			rets->at(i+1).at(index) = crypt(vals[i+1][j], fm->type, fullName(field, table), fullAnonName, getLevelForOnion(fm, rm.o[j]), getLevelPlain(rm.o[j]), salt,
					tmkm, vals[i+1]);
...
string EDBClient::processOperation(string operation, string op1, string op2, QueryMeta & qm, string  encryptedsubquery, TMKM & tmkm) throw (SyntaxError) {
	...
	if (!isField(op1)) {
		...
		if (MULTIPRINC) {
			assert_s(false, "MULTIPRINC first operand of operation should be field");
		}
		AES_KEY * aesKeyJoin = CryptoManager::get_key_DET(cm->getKey("join", DETJOIN));
		string res = "";
		res = marshallVal(cm->encrypt_DET((uint64_t) unmarshallVal(op1), aesKeyJoin)) + " IN " + encryptedsubquery + " ";
		return res;
	}
	...
	if (isField(op1) && isField(op2)) {//join
		...
		res = fieldNameForQuery(anonTable1, table1, field1, ftype1, qm) + " " + operation + " " + fieldNameForQuery(anonTable2, table2, field2, ftype2, qm) + " ";
	...
	if (Operation::isDET(operation)) { //DET
		...
		res = res + " " + fieldNameForQuery(anonTable1, table1, anonField1, ftype1, qm) + " " +  operation + " ";//operation remains unchanged
		...
		res = res + crypt(op2, ftype1, fullName(field1, table1), anonOp1, PLAIN_DET, highestEq(sl), 0, tmkm);
	...
	if (Operation::isILIKE(operation)) { //DET
		...
		assert_s(ftype == TYPE_TEXT, string("field ") + op1 + " has invalid type code " + marshallVal((unsigned int)ftype));

		res = res + crypt(op2, ftype, fullName(field1, table1), anonOp1, PLAIN_DET, DET, 0, tmkm);
	...
	if (DECRYPTFIRST) {
		...
		fieldname = fieldNameForQuery(anonTable1, table1, anonField1, ftype1, qm);
	}
...
list<const char*> EDBClient::rewriteEncryptDrop(const char * queryI) throw (SyntaxError){
	...
	assert_s(tableMetaMap.find(tableName) != tableMetaMap.end(), "the table to be deleted does not exist\n");
...
ResType * EDBClient::rewriteDecryptDrop(const char * query, ResType * dbAnswer) {
	if (VERBOSE) {cout << "Raw server response: \nDROP TABLE; \n\n\n";}
	return new ResType();  //no decryption performed, no need to waste time
}

list<const char*> EDBClient::rewriteEncryptDelete(const char * query) throw (SyntaxError) {

	/***********************
	 * QUERY looks like: DELETE FROM from_body WHERE where_body;
	 *
	 * -- resultQuery will hold the translated query
	 * -- the result of this function holds all the decryption queries needed and resultQuery
	 **********************/
	...
	assert_s(equalsIgnoreCase(*wordsIt, "delete"), "first token is not DELETE\n");
	wordsIt++;

	assert_s(toLowerCase(*wordsIt).compare("from") == 0, "second token of a DELETE statement is not FROM \n");
	wordsIt++;
	...
	while (!((wordsIt == words.end()) || (equalsIgnoreCase(*wordsIt, "where")))) {
		string table = *wordsIt;
		assert_s(tableMetaMap.find(table) != tableMetaMap.end(), "table does not exist");
		resultQuery = resultQuery + " " + tableMetaMap[table]->anonTableName + processAlias(++wordsIt, words);
	}

	FieldsToDecrypt ftd;
	list<const char *>  res = processFilters(wordsIt, words, qm, resultQuery, ftd, tmkm);
...
string EDBClient::processValsToInsert(string field, string table, uint64_t salt, string value, TMKM & tmkm) {
	...
	if (DECRYPTFIRST) {
		...
		if (type == TYPE_INTEGER) {
			...
			string res = marshallVal(CryptoManager::encrypt_DET(unmarshallVal(value), key));
			return res;
		} else {
...
string EDBClient::getInitValue(string field, string table, AutoInc * ai) {
	...
	if (tm->autoIncField == field) {
		//has autoinc
		assert_s(ai != NULL, "Current field has autoinc, but the autoincrement value was not supplied ");
...
list<const char*> EDBClient::rewriteEncryptInsert(const char * query, AutoInc * ai) throw (SyntaxError){
	...
	assert_s(tableMetaMap.find(table)!= tableMetaMap.end(), "table " + table + " does not exist");
	...
	if (wordsIt->compare("(") == 0) {
		...
		while (wordsIt->compare(")") != 0) {
			...
			resultQuery = resultQuery + " " + processInsert(*wordsIt, table, tm);
			wordsIt++;
			resultQuery += " " + checkStr(wordsIt, words, ",", ")");
		}

		// the user may not provide all fields in an insertion, expecting these to default
		// if some are encrypted fields we need to provide the encryption of default
		// if some of these are principals, we need to provide the next increment value
		noFieldsGiven = fields.size();

		TableMetadata * tm = tableMetaMap[table];

		for (addit = tm->fieldNames.begin(); addit!=tm->fieldNames.end(); addit++) {
			if (!contains(*addit, fields)) {
				...
				if (MULTIPRINC) {
					if (VERBOSE) {
						//cerr << "asking hasAccessTo " << fullName(*addit, table) << "and get hi ";
						//accMan->hasAccess(fullName(*addit, table)); cerr << "\n";
						//cerr << "asking hasAccessTo groupforum.forumid and get ";
						//myPrint(accMan->hasAccess("groupforum.forumid")); cerr << "\n";
					}
		...
		for (addit = fieldsToAdd.begin(); addit != fieldsToAdd.end(); addit++) {
		...
		if (MULTIPRINC) {
			...
			for (addit = princsToAdd.begin(); addit != princsToAdd.end(); addit++) {
				resultQuery += ", " + processInsert(*addit, table, tm);
				fields.push_back(*addit);
			}
		}
	...
	while (wordsIt != words.end()) {
			...
			if (MULTIPRINC) {
				...
				/*if (isPrincipal(fullname, accMan, mkm)) {
					if (autoInc.find(fullname) == autoInc.end()) {
						...
						autoInc[fullname] = max(autoInc[fullname], unmarshallVal(value));
						rb.autoinc = marshallVal(autoInc[fullname]);
						if (VERBOSE) { cerr << "rb autoinc is now " << rb.autoinc << "\n";}

					}
				}*/
			}

			//cerr << "processing for field " << *fieldIt << " with given value " << *valIt << "\n";
			resultQuery += processValsToInsert(*fieldIt, table, salt,  *valIt, tmkm);
			valIt++;
			fieldIt++;
		...
		for (addit = fieldsToAdd.begin(); addit != fieldsToAdd.end(); addit++) {
			...
			string val = *valIt;
			valIt++;
			resultQuery += ", " + processValsToInsert(field, table, salt, val, tmkm);
		}
...
ResType *  EDBClient::rewriteDecryptInsert(const char * query, ResType * dbAnswer) {
	return new ResType(); //no decryption performed, no need to waste time
}



list<const char*> EDBClient::rewriteEncryptCommit(const char * query) throw (SyntaxError) {
	...
	if (MULTIPRINC) {
		if (equalsIgnoreCase(*wordsIt, "annotations")) {
			wordsIt++;
			assert_s(wordsIt == words.end(), "nothing should come after <commit annotations>");
...
ResType * EDBClient::rewriteDecryptCommit(const char * query, ResType * dbAnswer) {
	return new ResType();
}



list<const char*> EDBClient::rewriteEncryptBegin(const char * query) throw (SyntaxError) {
...
ResType * EDBClient::rewriteDecryptBegin(const char * query, ResType * dbAnswer) {
	return new ResType();
}


list<const char*> EDBClient::rewriteEncryptAlter(const char * query) throw (SyntaxError) {
	...
	assert_s(wordsIt == words.end(), "expected end of query, but it continues " );
...
ResType * EDBClient::rewriteDecryptAlter(const char * query, ResType * dbAnswer) {
	return new ResType();
}
...
bool considerQuery(command com, const char * query) {
...
list<const char *> EDBClient::rewriteEncryptQuery(const char * query, AutoInc * ai)  throw (SyntaxError){
...
vector<vector<string> >  * EDBClient::decryptResults(const char * query, ResType * dbAnswer) {
...
void  EDBClient::exit(bool dropOnExit) {
...
string getQuery(ifstream & createsFile) {
	string line = "";
	string query = "";
...
int EDBClient::train(string queryFile) throw (SyntaxError){
...
int EDBClient::train_finish() throw (SyntaxError) {
	...
	for (; it != tableMetaMap.end(); it++) {
		...
		for (fieldIt = tm->fieldMetaMap.begin() ; fieldIt != tm->fieldMetaMap.end(); fieldIt++) {
...
int EDBClient::create_trained_instance(bool submit) throw (SyntaxError) {
	...
	for (; it != tableMetaMap.end(); it++) {
		...
		for (; fieldIt != tm->fieldNames.end(); fieldIt++) {
			...
			case TYPE_INTEGER: {
				...
				if (fm->exists(fm->anonFieldNameOPE)) {
					query = query + " " + fm->anonFieldNameOPE + " "+ TN_I64 + ",";
				}
				if (fm->exists(fm->anonFieldNameAGG)) {
			...
			default: {assert_s(false, "invalid type in create_trained_instance");}
			}
		}
	...
	for (; it != tableMetaMap.end(); it++) {
		...
		if (tm->primaryKey.size() > 0) {
			...
			for (; primIt != tm->primaryKey.end() ; primIt++) {
				...
				if (fm->exists(fm->anonFieldNameOPE)) {
					if (opelist.length() > 0) {
						cerr << "second ope in primary key for table " + it->first + " \n";
			...
			string pk_query = "ALTER TABLE " + tm->anonTableName + " add constraint con_" + tm->anonTableName + " primary key ( " +
					detlist + ");";
			cerr << pk_query << "\n";
			...
			if (opelist.length() > 0) {
				...
				string pk_ope_query = "CREATE INDEX inpk_" + tm->anonTableName +  " on " + tm->anonTableName + " (" + opelist + ");";
		...
		if (tm->indexes.size() > 0) {
			...
			for (; indexIt != tm->indexes.end(); indexIt++) {
				...
				index_query = index_query + " index " + im->anonIndexName + " on " + tm->anonTableName + " ( ";

				list<string>::iterator fieldIt = im->fields.begin();
				for (; fieldIt != im->fields.end(); fieldIt++) {
					index_query = index_query + tm->fieldMetaMap[*fieldIt]->anonFieldNameDET + " ,";
				}
...
void EDBClient::outputOnionState() {
	for (map<string, TableMetadata *>::iterator tm = tableMetaMap.begin(); tm != tableMetaMap.end(); tm++) {
		if (tm->second->hasEncrypted) {
			for (map<string, FieldMetadata *>::iterator fm = tm->second->fieldMetaMap.begin(); fm != tm->second->fieldMetaMap.end(); fm++) {
				FieldMetadata * f = fm->second;
				if (f->isEncrypted) {
					printf("%-36s", getCStr(fullName(f->fieldName, tm->first))); printf(" %-14s", getCStr(levelnames[f->secLevelDET]));
...
string EDBClient::crypt(string data, fieldType ft, string fullname, string anonfullname,
	...
	if (MULTIPRINC) {
		if (tmkm.processingQuery) {
			...
			if (VERBOSE_V) {
				...
				// cerr<< " tolevel " << tolevel << " salt " << salt << " key "; myPrint(key, AES_KEY_BYTES);
			}
	...
	string resu = cm->crypt(cm->getmkey(), data, ft, anonfullname, fromlevel, tolevel, salt);
	if (VERBOSE_V) {
		//cerr << "result is " << resu << "\n";
	}
	return resu;

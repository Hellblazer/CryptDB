 ...
 *      Author: raluca
 *
 *  Logic of translation between unencrypted and encrypted fields and manipulations of fields and tables.
 *
 */
...
QueryMeta getQueryMeta(command c, list<string> query, map<string, TableMetadata *> & tableMetaMap) throw (SyntaxError);

//it is made to point to the first field after select and distinct if they exist
string getFieldsItSelect(list<string> & words, list<string>::iterator & it);


bool isNested(const char * query);

//returns the name of a field if field is not encrypted, else it returns the DET name
...
//if the structure given is not of this form, it is considered a field with "" table
string getField(string tablefield);
string getTable(string tablefield);
bool isFieldSalt(string id);

//given a token representing a field, it returns the unanonymized table and field; these are original names and ignore aliases
// given token is unanonymized
void getTableField(string token, string & table, string & field, QueryMeta & qm, map<string, TableMetadata * > & tm) throw (SyntaxError);

//returns how a field should be called in a select anonymized query
// if table has alias, alias is used instead of anontabless
//if ignoreDecFirst is set to true, the function will behave as if DECRYPTFIRST were false
string fieldNameForQuery(string anontable, string table, string anonfield, fieldType ft,  QueryMeta & qm, bool ignoreDecFirst = false);
...
//if forquery is returned, it returns the entire aggregate expression with aliases as it should appear in the query
//if !forquery, it returns the name of this result as it should appear in the results
...
// if this clause contains only sensitive fields, it returns true and it remains unchanged
// if this clause contains only insensitive fields or non-field elements, it returns false and it points until after
...
bool processSensitive(list<string>::iterator & it, list<string> & words, string & res, QueryMeta & qm, map<string, TableMetadata *> & tm);
...
//adds in the respective lists queries for which decryption is needed at the server
...
/*
// input: a list of words corresponding to the WHERE clause of a query (all data after WHERE)
// effects/outputs: fieldsDec contains fields to be decrypted, whereClause contains the encrypted where clause, tm and fm are also updated
void processWHERE(list<string> words, TableMetadata * tm, FieldMetadata *fm, CryptoManager * cm, FieldsToDecrypt & fieldsDec, string & whereClause);
//
*/

...
#include <crypto/blowfish_data.h>
...
void
blowfish::block_encrypt(const uint8_t *ptext, uint8_t *ctext)
{
    BF_ecb_encrypt(ptext, ctext, &k, BF_ENCRYPT);
    // *(uint64_t*) ctext = bf60_encipher(&k, *(uint64_t*) ptext);
}

void
blowfish::block_decrypt(const uint8_t *ctext, uint8_t *ptext)
{
    BF_ecb_encrypt(ctext, ptext, &k, BF_DECRYPT);
    // *(uint64_t*) ptext = bf60_decipher(&k, *(uint64_t*) ctext);
}

blowfish::blowfish(const vector<uint8_t> &key)
{
    BF_set_key(&k, key.size(), &key[0]);
    // bf_setkey(&k, (void *) &key[0], key.size());
}
